...
The `home-routes` handler in the `budju.routes.home` namespace
...
The `home-page` function will in turn call the `budju.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `budju.handler` namespace:
...
Request middleware functions are located under the `budju.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `budju.dev-middleware` namespace found in
